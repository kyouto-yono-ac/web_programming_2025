...
| 10  | データ処理入門 (Pandas)               | Pandas基礎、DataFrame/Series、CSV読込、基本的な選択・フィルタリング、`st.dataframe` での表示         |                                        |
| 11  | 実践データ分析                       | Pandasを使ったデータ集計・加工（グループ化、結合など）、簡単な統計量の計算、`st.cache_data`/`st.cache_resource` による高速化 | 最終課題で扱うデータ分析の基礎固め   |
| 12  | データ可視化（基本と応用）             | Streamlit標準グラフ (`st.line_chart`, `st.bar_chart`等)、Plotly Express/Seaborn等を使ったグラフ作成、ライブラリ連携 | 分析結果を効果的に伝える               |
...
*   **小規模なデータ** を扱う（アプリ内で入力 or 事前定義した数件のデータ）。
